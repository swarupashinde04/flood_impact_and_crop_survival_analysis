...
⚙️ Methodology
A predefined dataset maps crops to their maximum flood survival duration.
User inputs:
Crop name
Flood duration (days)
The system:
...
MEDIUM RISK
HIGH RISK
Action-oriented recommendations are generated based on risk level.
🧠 Core Logic
If flood duration ≤ crop survival capacity → crop is SAFE
Else, survival probability is calculated as:
Risk thresholds:
≤ 40% → High Risk
41% – 70% → Medium Risk
70% → Low Risk
...
--- Crop Survival Assessment Report ---
Crop Name           : Rice
Flood Duration      : 10 days
Survival Capacity   : 12 days
